...
Kor\'{a}ndi and Sudakov    \cite{kor} also     noticed that  $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)=\mathrm{wsat}(K_n, K_s)$ with high probability when $p\geq n^{-\varepsilon}$ for small
...
Let   $X$ be a     random variable   with positive   expected value. If   $\mathbbmsl{Var}[X]=o(\mathbbmsl{E}[X]^2)$, then $X>0$  with high probability.
\end{corollary}
...
If $p\leq c_sq_s$, then   the property   $\mathcal{A}_s$ does not hold in $\mathbbmsl{G}(n, p)$ with high probability.
\end{theorem}


\begin{proof}
We first show that, for $p\ll n^{-2/(s+1)}$,   $\mathbbmsl{G}(n,p)$ does not have $\mathcal{A}_s$ with high probability. Let $\varepsilon\in(0,1)$ be fixed. By   Theorem \ref{chebyshev},
the number of edges of $\mathbbmsl{G}(n,p)$ belongs to
$(\tfrac{n^2p}{2+2\varepsilon},\tfrac{n^2p}{2-2\varepsilon})$  with high probability,   since it has binomial distribution with parameters ${n\choose 2}$ and $p$.
Let $X_s$ be the number of $K_s$  in $\mathbbmsl{G}(n,p)$. It is well known (see, e.g., Chapter 3 of \cite{RG}) that,
for $p\ll n^{-2/(s+1)}$,  $X_s=o(n^2p)$ with high probability, implying $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)=e(\mathbbmsl{G}(n, p))(1+o(1))$ with high probability,  since $e(\mathbbmsl{G}(n, p))-X_s\leq\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)\leq e(\mathbbmsl{G}(n, p))$. From this, we immediately get that   $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)\neq\mathrm{wsat}(K_n, K_s)$ with high probability for $p\ll n^{-2/(s+1)}$ such that $p\notin(\tfrac{2(1-\varepsilon)(s-2)}{n},\tfrac{2(1+\varepsilon)(s-2)}{n})$. For  such    $p$ and for any   $s\geq 4$,  $X_s=0$ with high probability and  therefore  the weak saturation number is exactly $e(\mathbbmsl{G}(n, p))$ that is not concentrated on a single value. Finally, if $p\in(\tfrac{2-2\varepsilon}{n}, \tfrac{2+2\varepsilon}{n})$, then, with high probability,   the weak $K_3$-saturation number equals the difference between the number of edges and the number of triangles  (with high probability, all the triangles in $\mathbbmsl{G}(n, p)$ are disjoint,  since, with high probability,  there are no subgraphs with at most $5$ vertices and at least two cycles; see Theorem 3.4 of \cite{RG}). The latter random variable is also not concentrated in a unit set. This is because the number of edges has the binomial distribution with parameters ${n\choose 2}$ and $p=\mathnormal{\Theta}(\tfrac{1}{n})$ (so,
for any interval of size $o(\sqrt{n})$, with high probability,  it is outside this interval) while the number of triangles is bounded from above by asymptotically Poisson random variable. In other words, for a constant $L$, the property of having at most $L$ triangles is {\sl decreasing} \cite[Chapter 1.3]{RG} while the asymptotic distribution of the number of  triangles in $\mathbbmsl{G}(n, \tfrac{2+2\varepsilon}{n})$ is Poisson; see Lemma 1.10 and Theorem 3.19 of \cite{RG}. Summing up, for $p\ll n^{-2/(s+1)}$,   $\mathbbmsl{G}(n,p)$ does not have $\mathcal{A}_s$ with high probability.
...
\begin{enumerate}
\item[{\rm (i)}] If $w_n\to\infty$, then   $\mathbbmsl{G}(n,p)$ has $\mathcal{B}_s$ with high probability.
\item[{\rm (ii)}] If $w_n\to-\infty$, then   $\mathbbmsl{G}(n,p)$ does not have $\mathcal{B}_s$ with high probability.
\end{enumerate}
\end{lemma}
...
If $p\gg r_s$, then  it follows from   Theorem \ref{lower-ii} that   $\mathbbmsl{G}(n,p)$ has the property $\mathcal{A}_s$ with high probability. It remains to prove the opposite for $p\ll r_s$.
If $p(n)\leq c_sq_s(n)$ for infinitely many $n$, then  it follows from    Theorem \ref{lower-ii}  that  $\mathbbmsl{G}(n, p)$ does not have the property $\mathcal{A}_s$ with high probability. So, we may assume $p(n)>c_sq_s(n)$ for all large enough integer  $n$.   If $(1+\varepsilon)q^*_s(n)\leq p(n)\ll r_s(n)$  for  some fixed  $\varepsilon>0$ and  infinitely many $n$, then
...
The {\sl $k$-th power}  of a graph  $\mathnormal{\Gamma}$, denoted by $\mathnormal{\Gamma}^k$,  is the graph  with    vertex set $V(\mathnormal{\Gamma})$  such that two vertices $x, y$ are adjacent in $\mathnormal{\Gamma}^k$ if and only if the distance between  $x, y$  in  $\mathnormal{\Gamma}$  is at most $k$.
...
Let  $s\geq 3$ be  a fixed  integer and $p\geq n^{-1/(2s-3)}(\ln n)^{2}$. Then,  $$\mathbbmsl{P}[\text{$\mathbbmsl{G}(n, p)$  has both properties $\mathsf{EXT}$ and $\mathsf{HAM}$}]\to1$$ as $n\to\infty$.
\end{lemma}



\begin{proof}
By Theorem 2 of \cite{SP90},  $\mathbbmsl{G}(n,p)$ has the property {\sf EXT} with high probability provided that    $$p\gg n^{-\frac{2}{3(s-1)}}(\ln n)^{\frac{2}{3(s-2)(s-1)}}.$$
Therefore, if  $p\geq n^{-1/(2s-3)}(\ln n)^{2}$, then $\mathbbmsl{P}[\text{$\mathbbmsl{G}(n, p)$  has the  property $\mathsf{EXT}$}]\to1$ as $n\to\infty$.
To explore the  behavior of  $\mathbbmsl{P}[\text{$\mathbbmsl{G}(n, p)$  has the  property $\mathsf{HAM}$}]$, we use the    fact that, for every   positive integers $k$ and $r$, if    $m$ is sufficiently large and  $q(m)\gg m^{-1/k}(\ln m)^4$,  then $\mathbbmsl{G}(m, q)$ contains the $k$-th power of a Hamiltonian path    with probability at least $1-m^{-r}$.
To see  this fact, we may  combine   Theorem 2 of \cite{posa}, Theorem 1.6 of \cite{fischer}  and Theorem 1.1 of \cite{kuhn} to  conclude   that
there exists a constant $C>0$ such that if $\widecheck{q}(m)>Cm^{-1/k}(\ln m)^3$, then $\mathbbmsl{G}(m, \widecheck{q})$ contains the $k$-th power of a Hamiltonian path   with probability at least $1-\tfrac{1}{e}$.
...
In Theorem 1.3 of \cite{kor},  Kor\'{a}ndi  and Sudakov proved, if  $p\in(0, 1)$  is a   constant probability  and    $s\geq3$ is   a fixed  integer, then  $\mathrm{wsat}(\mathbbmsl{G}(n,p), K_s)={s-2\choose 2}+(s-2)(n-s+2)$ with high probability.  The following theorem  generalizes    their  result.



\begin{theorem}\label{th_upper}
Let  $s\geq 3$ be  a fixed  integer and $p\geq n^{-1/(2s-3)}(\ln n)^2$. Then,  $\mathbbmsl{G}(n,p)$ has the property $\mathcal{A}_s$ with high probability.
\end{theorem}
...
\begin{proof}
Using    Theorem 1 of  \cite{bal},    $\mathbbmsl{G}(n,p)$ is a weakly $K_s$-saturated subgraph of  $K_n$ with high probability and thus    we get  that $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)\geq\mathrm{wsat}(K_n, K_s)={s-2 \choose 2}+(s-2)(n-s+2)$  with high probability.
It remains to prove that  there exists a weakly $K_s$-saturated subgraph of $\mathbbmsl{G}(n,p)$ of size   ${s-2\choose 2}+(s-2)(n-s+2)$    with high probability. So,
...
The power of  the logarithm  factor in Theorem \ref{th_upper} is not the best possible. Using best known estimations of threshold probability for containing the $k$-th power of a Hamiltonian path, we may replace the factor
$(\ln n)^2$    in Theorem \ref{th_upper} with $(\ln n)^\xi$, where
$$\xi=\left\{\begin{array}{ll}
\frac{1}{3}+\varepsilon  &   \mbox{ if } s=3\mbox{,}\\\vspace{-2mm}\\
\frac{8}{5} &  \mbox{ if } s=4\mbox{,}\\\vspace{-2mm}\\
\frac{1}{2} &  \mbox{ if } s\geq5\mbox{.}
\end{array}\right.$$
\end{remark}
...
From the previous arguments, we know that  $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)=e(\mathbbmsl{G}(n,p))(1+o(1))$ with high probability when $p(n)\ll n^{-2/(s+1)}$ and $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)={s-2\choose 2}+(s-2)(n-s+2)$  when $p(n)\geq n^{-1/(2s-3)}(\ln n)^{\beta}$, where $\beta=\tfrac{1}{3}+\varepsilon$ for $s=3$, $\beta=\tfrac{8}{5}$ for $s=4$ and $\beta=\tfrac{1}{2}$ for $s\geq 5$. In this section, we prove an upper bound for $\mathrm{wsat}(\mathbbmsl{G}(n, p), K_s)$ for all the remaining values of $p$ (the trivial lower bound equals ${s-2\choose 2}+(s-2)(n-s+2)$ with high probability,  since $\mathbbmsl{G}(n,p)$ is weakly saturated in $K_n$ for those values of $p$ with high probability \cite{bal}).
...
1  &   \mbox{ if } s=3\mbox{,}\\
6 &  \mbox{ if } s=4\mbox{,}\\
0 &  \mbox{ if } s\geq5\mbox{.}
\end{array}\right.$$
Also, let  $w_1, w_2, \ldots$ be a sequence of real numbers such that   $w_n\to\infty$ as $n\to\infty$. Then,      $$\mathrm{wsat}\big(\mathbbmsl{G}(n, p), K_s\big)\leq (s-2)n+\frac{w_n(\ln n)^{2(\gamma+s-2)}}{p^{2s-3}}$$ with high probability.
\end{theorem}



\begin{proof}
The result is immediate when     $p\leq((\ln n)^{\gamma+s-2}/n)^{1/(s-1)}$,   since   in this case  $e(\mathbbmsl{G}(n,p))\ll w_n(\ln n)^{2(\gamma+s-2)}/p^{2s-3}$ with high probability. If  $p\geq n^{-1/(2s-3)}(\ln n)^\xi$  for    $\xi$  presented  in Remark \ref{rmrk}, then   the result follows from Remark \ref{rmrk}. So,  we may assume that
...
Let $G\sim\mathbbmsl{G}(n, p)$ and  let  $X=[\![m]\!]$, where $m=\lfloor(\ln n)^{\gamma+s-2}\sqrt{w_n}/p^{s-1}\rfloor$.   Since   $e(G[X])\leq m^2p$ with high probability, it remains to show that  the properties   (i) and (ii) hold  with high probability  for $G$ and our choice of $X$.


By combining   Theorem 2 of \cite{posa}, Theorem 1.6 of \cite{fischer}  and Theorem 1.1 of \cite{kuhn}, we   conclude  that,   if    $\ell$ is sufficiently large and     $p\gg\ell^{-1/(s-2)}(\ln n)^{\gamma/(s-2)+1}$, then $\mathbbmsl{G}(n,p)[\ell]$ contains the $(s-2)$-th power of a Hamiltonian path with probability at least $1-o(\tfrac{1}{n})$. Given a vertex $v\in[\![n]\!]\setminus X$, with probability $o(\tfrac{1}{n})$, $v$ has  at most  $\lfloor(\ln n)^{\gamma+s-2}\sqrt[4]{w_n}/p^{s-2}\rfloor$ neighbors in $X$ using  Theorem \ref{chernoff}. This shows that  the property     (i) holds with
high probability.
...
\bibitem{kuhn} {\sc D. K\"{u}hn}  and {\sc D. Osthus}, On P\'{o}sa’s conjecture for random graphs, {\sl SIAM J. Discrete Math.}  {\bf 26} (2012), 1440--1457.
...
By Theorem 2 of  \cite{SP90},  there exists a constant $C>0$ such that, if    $p\geq Cq_s$,   then  every pair of vertices of  $\mathbbmsl{G}(n,p)$     have  with high probability  $s-2$ common neighbors that induce a clique. Thus, we may assume that $w_n\leq C\ln n$.
